...
function createCityGraphic(){
    ...
    function gameOverHandler(){
    // stop the game loop visuals
    context.clearRect(0, 0, canvas.width, canvas.height);

    // make sure this only runs once
    if (!window._confessionTriggered) {
        window._confessionTriggered = true;

        // give a quick "crash" effect
        document.body.style.animation = "shake 0.5s";

        // optional: make the canvas white for dramatic effect
        context.fillStyle = "#FFFFFF";
        context.fillRect(0,0,canvas.width, canvas.height);

        // stop game updates (safer)
        clearTimeoutsAndIntervals();

        // redirect almost immediately (short pause so shake is visible)
        setTimeout(function(){
            // change this to your actual confession URL
            window.location.href = "the-art-of-love-confessions/dist/index.html";
        }, 450); // 450ms lets the shake play once then redirect
    } else {
        // if somehow called again, just show game over screen
        gameState = GAME_OVER;
        renderGameOver();
    }
}
